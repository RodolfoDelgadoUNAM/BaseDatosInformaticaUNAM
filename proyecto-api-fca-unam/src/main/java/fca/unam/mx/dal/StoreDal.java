...
public class StoreDal {
    ...
    //agregue este metodo y modifique su contenido
    public ResponseDto<List<ProductDto>> getClients() {

        ResponseDto responseDto = new ResponseDto<List<ClientDto>>();//cambio por ClientDto
        responseDto.setSuccess(true);
        Jdbi jdbi = jdbiService.getInstance();
        var clients = jdbi.withExtension(StoreDao.class, dao -> dao.getClients());//por Clients y getClients
        responseDto.setData(clients);//cambie por clients
        return responseDto;
    }
